...
function($) {
    ...
    var getBulkImageData = function() {
            ...
            return $rows.each(function() {
                $row = $(this), postId = this.id.replace(/^\D+/g, ""), $row.find("input[type='checkbox'][value='" + postId + "']:checked").length && ($enjoyedBtn = $row.find(".way2enjoy_req"), $enjoyedBtn.length && (btnData = $enjoyedBtn.data(), originalSize = $.trim($row.find("td.original_size").text()), yettooptimise = $.trim($row.find("td.yetto_opt").text()), btnData.originalSize = originalSize, btnData.yettooptimise = yettooptimise, rv.push(btnData)))
            }), rv
        },
        bulkModalOptions = {
            zIndex: 4,
		...
		//	console.log(setting1);	
					$('#auto_optimize_bulk_dummy2').on('change', function() {
					$('input#auto_optimize_bulk').click(); 
					});
            ...
            $table.append($headerRow), $.each(bulkImageData, function(index, element) {
                $table.append('<tr class="way2enjoy-item-row" data-way2enjoybulkid="' + element.id + '"><td class="way2enjoy-bulk-filename">' + element.filename + '</td><td class="way2enjoy-originalsize">' + element.originalSize + '</td><td class="yet_to_optimize2" style="display:none">' + element.yettooptimise + '</td><td class="way2enjoy-way2enjoyedsize"><span class="way2enjoyBulkSpinner hidden"></span></td></tr>')
            ...
            var jqxhr = null,
                q = async.queue(function(task, callback) {
                    var id = task.id,
						...
						$yettooptcount = $row.find(".yet_to_optimize2").text(),
                    ...
                    jqxhr = $.ajax({
                        ...
                        if (data.success && "undefined" == typeof data.message) {
                            var originalSize = (data.type, data.original_size),
								...
								yettooptimize99 = totalimgtoopt-$yettooptcount,
                            ...
                            var $button = $("button[id='way2enjoyid-" + id + "']"),
								...
								var	ramukaka=null;
								var randomno=Math.floor(Math.random() * 30);
								if(quotaRema===undefined){ramukaka='' + wp_way2_msgs.buy_msg + '';}else{
									if(randomno<='29')
									{
									ramukaka='' + wp_way2_msgs.quotabal_dtls + ' '+quotaRema+' • '+yettooptimize99+' ' + wp_way2_msgs.yet_tooopttimi + '';	
									}
									else
									{
									ramukaka='' + wp_way2_msgs.schd_optway + '';	
		...
		$(".way2enjoyBulkHeader1").text(ramukaka);	
            ...
            q.drain = function() {
			//	setTimeout(function() {$(".way2enjoy_req_bulk").trigger('click');location.reload();}, 1000);
