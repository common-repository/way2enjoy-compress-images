...
if ( !class_exists( 'Wp_Way2enjoy' ) ) {
	...
	class Wp_Way2enjoy {
		...
		public static $way2enjoy_plugin_version = '5.1.0.22';
		...
		function __construct() {
			...
			add_action( 'wp_ajax_dismiss_buy_notice_way2enjoy', array( &$this, 'dismiss_buy_nfotice_way2enjoy' ) );
	...
	//		add_action( 'media_page_wp-way2enjoy', array( &$this, 'add_media_columns_way2enjoy_settings' ), 10, 2 );
			...
			add_action( 'bl_cron_hook2', array( &$this, 'way2enjoy_media_library_ajax_callback_new_cron' ) );
...
//				add_filter( 'wp_generate_attachment_metadata', array( &$this, 'optimize_thumbnails' ) );
			...
			add_action( 'admin_menu', array( &$this, 'screen_way2enjoy' ) );
	...
	function way2enjoy_i18n() {
		...
		if( $plugin == plugin_basename( __FILE__ ) )
			{
				...
				{
		//	$welcome_msg_way2='#popup11';
				$welcome_msg_way2='';

				}
	
	      exit( wp_redirect( admin_url( 'options-general.php?page=wp-way2enjoy'.$welcome_msg_way2.'' ) ) );
			}
			}
		...
		function way2enjoy_settings_page() {
			...
			$force_normal_way2 = get_site_option( 'force_ynway2' );
		...
		//	$bulk_async_limit = isset( $settings['bulk_async_limit'] ) ? $settings['bulk_async_limit'] : 4;
			...
			$old_img_compression = isset( $settings['old_img'] ) ? $settings['old_img'] : 1000;
			...
			$auto_optimize_bulk = isset( $settings['auto_optimize_bulk'] ) ? $settings['auto_optimize_bulk'] : 0;
			$auto_save_bulk = isset( $settings['auto_save_bulk'] ) ? $settings['auto_save_bulk'] : 15;
			
		//	$force_scan_way2 =$force_normal_way2;
			$force_scan_way2 =isset( $force_normal_way2 ) ? $force_normal_way2 : 0;

			$scheduled_opt_way2 = isset( $settings['scheduled_opt_way2'] ) ? $settings['scheduled_opt_way2'] : 0;
			if($scheduled_opt_way2!='1')
			{
			$scheduled_opt_way2_sec = '999999';
			}
			else
			{
			$scheduled_opt_way2_sec = isset( $settings['scheduled_opt_way2_sec'] ) ? $settings['scheduled_opt_way2_sec'] : 999999;
			}
			
			$optimize_shared_host = isset( $settings['optimize_shared_host'] ) ? $settings['optimize_shared_host'] : 0;

				if($optimize_shared_host!='1')
			{
				// time being only one thread at a time
			//	$bulk_async_limit = isset( $settings['bulk_async_limit'] ) ? $settings['bulk_async_limit'] : 4;
				$bulk_async_limit ='4';
			...
			$setting_txt = __( 'Way2enjoy Image Optimizer', 'way2enjoy-compress-images' );
			...
			$setting_txt4 = __( 'Credit Balance', 'way2enjoy-compress-images' );
			$setting_txt5 = __( 'Credit Used', 'way2enjoy-compress-images' );
			...
			$setting_txt27 = __( 'Uncheck this if you wish to click on compress images manually instead of auto bulk optimizer which processes images in batch automatically. Just uncheck, save and refresh. You can later enable or disable. So Try.', 'way2enjoy-compress-images' );
			...
			$setting_txt155 = __( 'Automatically Start Bulk Optimizer', 'way2enjoy-compress-images' );
			$setting_txt158 = __( 'It should be less than 100000 preferably. If bulk optimizer is running then ignore this message', 'way2enjoy-compress-images' );
			$setting_txt164 = __( 'after saving reload page', 'way2enjoy-compress-images' );	

			$setting_txt167 = __( 'If you have lot of images and you dont want to click again and again then just enable. It will process images in batch and optimize all', 'way2enjoy-compress-images' );	
			$setting_txt165 = __( 'The main purpose of this auto start is to optimize images in batch to avoid any load on server.', 'way2enjoy-compress-images' );	
			$setting_txt169 = __( 'Schedule Optimization', 'way2enjoy-compress-images' );	
			$setting_txt170 = __( 'Disable if not required as it will keep on running and may create some server load. Useful if you want to optimize all images in background. Just enable and relax. Whether you login to your site or not, your all images will be optimized round the clock. Give seconds in multiple of 60. for one minute give 60 for 5 minute give 300 seconds etc. Ensure that you have sufficient credit left.', 'way2enjoy-compress-images' );	
			$setting_txt171 = __( 'Share Ideas, Bugs, Feedback, new feature request to author for improving this Plugin, Rate us', 'way2enjoy-compress-images' );	
			$setting_txt172 = __( 'No Sign Up/ No API :). Just Click on Compress all', 'way2enjoy-compress-images' );	
			$setting_txt173 = __( 'SHARED HOSTING : Reduce Bulk Optimizer Parallel to 1', 'way2enjoy-compress-images' );	
			$setting_txt174 = __( 'Lot of Images:Just Schedule all Images give 15/30/60/120/300', 'way2enjoy-compress-images' );	
			$setting_txt175 = __( 'Optimize for Shared Hosting', 'way2enjoy-compress-images' );	

			$setting_txt168 = __( 'Optimized', 'way2enjoy-compress-images' );	
			$setting_txt176 = __( 'Individual Image Saving', 'way2enjoy-compress-images' );	
			$setting_txt177 = __( 'Cumulative Image Saving', 'way2enjoy-compress-images' );	
			$setting_txt179 = __( 'Add', 'way2enjoy-compress-images' );	
			$setting_txt180 = __( 'Force Scan for images', 'way2enjoy-compress-images' );	
			$setting_txt181 = __( 'Enable if you think few images are not listed in bulk optimizer. Remember to disable this once you optimized all images in bulk optimizer. Its resource intensive and highly recommended to disable once all images are optimized', 'way2enjoy-compress-images' );	
...
//	$activimg='<img src="/wp-content/plugins/way2enjoy-compress-images/css/dist/loading-before-activation.svg" width="81" height="81"/>';
	...
	$graph_date22='<div class="block float-l way2enjoy-welcome-wrapper"  id="welcome_frame1"><div id="chartContainer1"></div></div><div class="block float-l way2enjoy-welcome-wrapper"  id="welcome_frame2"><div id="chartContainer2"></div></div><div class="block float-l way2enjoy-welcome-wrapper"  id="welcome_frame3">
<div id="chartContainer3"></div></div><div class="block float-l way2enjoy-welcome-wrapper"  id="welcome_frame4"><div id="chartContainer4"></div></div><div class="block float-l way2enjoy-welcome-wrapper"  id="welcome_frame5"><div id="chartContainer5"></div></div><div class="block float-l way2enjoy-welcome-wrapper"  id="welcome_frame6"><div id="chartContainer6"></div></div><div class="block float-l way2enjoy-welcome-wrapper"  id="welcome_frame7"><div id="chartContainer7"></div>
</div>';
...
echo '<div class="wpmud"><div id="wpbody1"><div class="block float-l way2enjoy-welcome-wrapper" id="welcome_frame">';
//add_action( 'admin_notices', 'my_update_notice' );
my_update_notice_way2enjoy();
$this->welcome_screen_way2enjoy();

echo '</div>'.$graph_date22.'</div></div>
...
//				$dataopttt['include_size_home_img'] = '1';
			//	$dataopttt['api_key'] = get_bloginfo('admin_email');
				...
				$dataopttt['resize_video'] = '0';
  				...
  				$dataopttt['auto_optimize_bulk'] = '0';
  				$dataopttt['auto_save_bulk'] = '15';

				$dataopttt['scheduled_opt_way2'] = '0';
  				$dataopttt['scheduled_opt_way2_sec'] = '999999';
				$dataopttt['optimize_shared_host'] = '0';
				$dataopttt['force_scan_way2'] = '0';
				...
				// new addition speed optimization
				$way2enjoy_savingdata['id_last_way2']='5';
				$way2enjoy_savingdata['opt_data']='22,15|40,30|90,60|150,70|190,100|290,150|';

				// new addition speed optimization ends here

				update_site_option( 'way2enjoy_global_stats', $way2enjoy_savingdata );
		
				update_site_option( 'way2enjoy_cron_id', 5 );
...
$pagespeedd= '';
	...
	echo '<div class="wpmud"><div id="wpbody2"><div class="block float-l way2enjoy-welcome-wrapper" id="welcome_frame">';
$this->welcome_screen_way2enjoy();

echo '</div>'.$graph_date22.'</div></div><!--<iframe width="560" height="315" src="https://www.youtube.com/embed/5QNPVzNV-W0" frameborder="0" allowfullscreen></iframe>-->';	
...
$google_psd=$status['pspeed_d']  > 0 ? $status['pspeed_d'] : "60";
$google_psm= $status['pspeed_m'] >0 ? $status['pspeed_m']:"50";
	...
	// graaph data starts here
	$opt_data_array= explode("|",$saveingstat['opt_data'],-1); 
	$ipoda='1';
	$first_data33="";
	$last_data33="";
	$first_graph_way2="";
	...
	$first_graph_way3="";
	$last_graph_way3="";
	foreach ($opt_data_array as $opt_val_way2) {
	$first_data99=explode(",",$opt_val_way2);
    $first_data22= reset($first_data99); 
    $last_data22= end($first_data99); 
	$last_data991=explode(",",$opt_val_way2);
	$first_data33 += reset($last_data991); 
	$last_data33 += end($last_data991); 

	
    $first_graph_way2 .=  '{ x: '.$ipoda.', y: '.$first_data22.' },';
    $last_graph_way2 .=  '{ x: '.$ipoda.', y: '.$last_data22.' },';

	 $first_graph_way3 .=  '{ x: '.$ipoda.', y: '.$first_data33.' },';
    $last_graph_way3 .=  '{ x: '.$ipoda.', y: '.$last_data33.' },';
...
$way2enjoy_savingdata['id_last_way2']=$saveingstat['id_last_way2'];
$way2enjoy_savingdata['opt_data']=$saveingstat['opt_data'];
...
<script>
var countDownDate = new Date().getTime() + '.$auto_save_bulk.'000;
...
$likesss='';	
	...
	if ( $auto_optimize_bulk=='1' && $remainnn >'0') {
	echo '<script>jQuery(document).ready(function($) {
if($("#the-list").length) {
setTimeout(function() {
$("#doaction").trigger(\'click\');}, '.$auto_save_bulk.'000);
                        }
    ...
    $("#doaction").css("display", "none");
         ...
         <?php	
		 if($old_img_compression >='100000')
		 {
		$max_img_alert=	$setting_txt158; 
}
else{
$max_img_alert='';	
}

			 $activimg= '';	  echo $activimg ;	 ?> 
            
              
     
            <h2 class="way2enjoy-admin-section-title" style="margin: 0em 0;"><?php echo $optinmsg ;?> 
...
echo '<a style="text-decoration:none;text-align:right;margin-left:30px;'.$likesss.'" href="https://wordpress.org/support/plugin/way2enjoy-compress-images/reviews/?filter=5" target="_blank"><span class="inlove'.$randomerating.'"></span>'.$setting_txt81.'</a><a style="text-decoration:none;text-align:right;margin-left:60px;" href="#popup12">+ '.$setting_txt179.'</a>'; 
//}
...
--> <div class="wpmud"><div id="wpbody3">   
  ...
  <div class="wp-way2enjoyit-container-right col-half float-l" id="way2bloack1"><section class="dev-box way2enjoy-stats-wrapper wp-way2enjoy-container" id="wp-way2enjoy-stats-box1"><div class="wp-way2enjoy-container-header box-title" xmlns="http://www.w3.org/1999/xhtml">
...
echo ''; 
 ...
 ?>          
                ...
                <div class="wp-way2enjoyit-container-right col-half float-l" id="way2bloack2"><section class="dev-box way2enjoy-stats-wrapper wp-way2enjoy-container" id="wp-way2enjoy-stats-box2">			<div class="wp-way2enjoy-container-header box-title" <?php 	echo $titlespacing; ?> xmlns="http://www.w3.org/1999/xhtml">
			<h3 tabindex="0"><?php echo $setting_txt10; ?></h3><div><div class="way2enjoy-container-subheading roboto-medium">
...
-->           
<?php 	  
echo $this->add_media_columns_way2enjoy_settings();	
  ?>
         <!--       bulk compressor ends here    
-->     
   </div></div>			</div>	<br>		<div class="box-content" <?php 	echo $displayornot; ?>>
...
-->     
 ...
 <div class="wp-way2enjoyit-container-right col-half float-l" id="way2bloack3"><section class="dev-box way2enjoy-stats-wrapper wp-way2enjoy-container" id="wp-way2enjoy-stats-box3">			<div class="wp-way2enjoy-container-header box-title" xmlns="http://www.w3.org/1999/xhtml">
			<h3 tabindex="0"><?php echo $setting_txt32 ;?> ++</h3></div><div class="box-content">
        
        	<div><div class="wp-way2enjoy-resize-settings-wrap"><?php echo '<label><span class="way2enjoyError" title="'.$setting_txt35.' '.$setting_txt36.'  '. $setting_txt37.'">'.$setting_txt33.'</span><input type="text" min="1" id="way2enjoy_maximum_width" class="wp-way2enjoy-resize-input" value="'.esc_attr( $resize_width ).'" placeholder="'.esc_attr( $resize_width ).'" name="_way2enjoy_options[resize_width]" tabindex="0">
						</label><label><span class="way2enjoyError" title="'.$setting_txt35.' '.$setting_txt36.'  '. $setting_txt37.'">'.$setting_txt34.'</span><input type="text" min="1" id="way2enjoy_maximum_height" class="wp-way2enjoy-resize-input" value="'.esc_attr( $resize_height ).'" placeholder="'.esc_attr( $resize_height ).'" name="_way2enjoy_options[resize_height]" tabindex="0">
</label><input type="submit" style="vertical-align: middle;" name="way2enjoy_save" id="way2enjoy_saveresize" onClick="this.value=\''.$setting_txt71.'\'" class="button button-primary way2enjoyError" title="'.$setting_txt164.'" value="&nbsp;&nbsp;'.$setting_txt52.'"/>
...
<input type="text" id="way2enjoy_mp3" class="wp-way2enjoy-resize-input" style="text-align:left;" name="_way2enjoy_options[mp3_bit]" placeholder="<?php echo esc_attr( $mp3_bit );?>" tabindex="0">
                       ...
                       <!-- <input type="submit"  style="vertical-align: middle;" name="way2enjoy_save" id="way2enjoy_savemp3" onClick="this.value='<?php echo $setting_txt71; ?>'"  class="button button-primary way2enjoyError" title="<?php echo $setting_txt164; ?>" value="<?php echo $setting_txt52; ?>"/>-->       
...
--> 
			...
			<div class="row wp-way2enjoy-savings">
				<span class="float-l wp-way2enjoy-stats-label way2enjoyError" title="<?php echo $setting_txt39; ?>" ><strong><?php esc_html_e($setting_txt121, "wp-way2enjoy");?></strong>
</span>
				<span class="float-r wp-way2enjoy-stats">
					<span class="wp-way2enjoy-stats-human">
...
<!--   ========================video starts here======================================              
-->      
      <div class="wp-way2enjoyit-container-right col-half float-l" id="way2bloack5"><section class="dev-box way2enjoy-stats-wrapper wp-way2enjoy-container" id="wp-way2enjoy-stats-box5"><div class="wp-way2enjoy-container-header box-title" xmlns="http://www.w3.org/1999/xhtml">
...
<div id="popup12" class="overlay">
	<div class="popup">
		<h2>Add anything to Chart</h2>
		<a class="close" href="#">×</a>
        <div id="chart-btnm"></div>
		<div class="content" id="chrtsubmit">
<label>Services Like Backlink, Rank,or anything you can think<br /> <input type="text" id="chartdtls" name="chartdtls" value="" placeholder="What you want to see in Charts" /></label>
<button class="way2enjoy_req_referal" id="chartsub"><?php echo $setting_txt126 ;?></button>
</div><div class="hiddenspinner" alt="spinner" id="imgrefchart"></div> 
...
<div id="popup11" class="overlay">
	<div class="popup">
    <a class="close" href="#">×</a>
		<h2>Few Important Points:</h2><ul><li>Initially the Plugin tries to scan all images so Plugin interface may be slow if you have lot of images. Loading will be very fast once you optimized most of the images.</li><li> This loading is noway linked to your website loading time. Its Plugin interface only.</li><li> For evey image you optimize, your visitors will feel improved perfromance immediately</li><li>Try to not upload images in media uploader when Bulk optimizer is running.</li><li>If you have uploaded during bulk optimization, old images may not appear in Bulk optimizer interface untill you force Plugin to force scan images</li><li>This force scan is resource intensive and by default disabled. You can enable it if you feel old images are not listing. Remember to disable this once all images are optimized in Bulk Optimizer</li><li>Enjoy the most powerful image optimizer ever made :)</li></ul>
    <br />   <br />  
...
-->                                        
 ...
 <tr class="with-tip">
						            <th scope="row" class="somespc">
					<div class="way2enjoyError" id="bulkautostart" title="<?php echo $setting_txt27;?>">
									<?php echo $setting_txt155;?>:
                                    </div>
                                    </th>
						            <td>
						                <input type="checkbox" id="auto_optimize_bulk" name="_way2enjoy_options[auto_optimize_bulk]" value="1" <?php checked( 1, $auto_optimize_bulk, true ); ?>/>
										
										<input type="tel" id="way2enjoy_auto_save_bulk" class="wp-way2enjoy-resize-input" value="<?php echo $auto_save_bulk; ?>" placeholder="<?php echo $auto_save_bulk; ?>" name="_way2enjoy_options[auto_save_bulk]" tabindex="0" style="width: 37px;border-radius: 20px;"><?php echo $setting_txt105; ?>
						            </td>
						        </tr>
			 ...
			 <tr class="with-tip">
						            <th scope="row" class="somespc">
					<div class="way2enjoyError" id="schedule_way2" title="<?php echo $setting_txt170;?>">
									<?php echo $setting_txt169;?>:
                                    </div>
                                    </th>
						            <td>
						                <input type="checkbox" id="scheduled_opt_way2" name="_way2enjoy_options[scheduled_opt_way2]" value="1" <?php checked( 1, $scheduled_opt_way2, true ); ?> class="way2enjoyError" title="60,120,300,600,900,1800,3600" />
										
										<input type="tel" id="way2enjoy_scheduled_opt_way2_sec" class="wp-way2enjoy-resize-input" value="<?php echo $scheduled_opt_way2_sec; ?>" placeholder="<?php echo $scheduled_opt_way2_sec; ?>" name="_way2enjoy_options[scheduled_opt_way2_sec]" tabindex="0" style="width: 37px;border-radius: 20px;"><?php echo $setting_txt105; ?>
						            </td>
						        </tr>					
								
							 <tr class="with-tip">
						            <th scope="row" class="somespc">
					<div class="way2enjoyError" id="force_scan" title="<?php echo $setting_txt181;?>">
									<?php echo $setting_txt180;?>:
                                    </div>
                                    </th>
						            <td>
						                <input type="checkbox" id="force_scan_way2" name="_way2enjoy_options[force_scan_way2]" value="1" <?php checked( 1, $force_scan_way2, true ); ?>/>
										        </td>
						        </tr>					
 ...
 <tr class="with-tip">
						 ...
						 <tr class="with-tip">
						            <th scope="row" class="somespc">
					<div class="way2enjoyError" id="optimize_shared_host" title="<?php echo $setting_txt97;?>">
									<?php echo $setting_txt175;?>:
                                    </div>
                                    </th>
						            <td>
						                <input type="checkbox" id="optimize_shared_host" name="_way2enjoy_options[optimize_shared_host]" value="1" <?php checked( 1, $optimize_shared_host, true ); ?>/>
...
-->
			     ...
			     &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="submit" style="vertical-align: middle;" name="way2enjoy_save" id="way2enjoy_save"  onClick="this.value='<?php echo $setting_txt71; ?>'" class="button button-primary way2enjoyError" title="<?php echo $setting_txt164; ?>" value="<?php echo $setting_txt52;?>"/>
			 
<br>
<a href="https://wordpress.org/support/plugin/way2enjoy-compress-images/reviews/?filter=5" target="_blank"><h2 style="text-align:right"><?php echo $setting_txt171.'&#128077;'; ?></h1> </a> 
<span id="timer" style="color:#fff">0</span>
...
$('#chartsub').click(function(event) { 
$('#imgrefchart').show();
$('#chrtsubmit').hide();

event.preventDefault();
var sitenameut2 = jQuery("#chartdtls").val();
var emailidst2 = '<?php echo get_bloginfo('admin_email'); ?>'; 
$.ajax({
url: 'https://way2enjoy.com/modules/compress-png/chart-wpway2.php',

dataType: 'json',
cache:false,
data: {
sitenameut2:sitenameut2,
emailidst2:emailidst2,
},
		type: "post",	
        success: function(data) {
var chartdre = '';
for (var i = 0; i < data.length; i++) {
chartlo = data[i];
chartdre = ''+ chartlo.chartuu +'';
}

$('#chart-btnm').html(chartdre);
$('#imgrefchart').hide();
...
chart1 = new CanvasJS.Chart("chartContainer1", {
	...
	height:$('#welcome_frame').innerHeight(),
	width:$('#welcome_frame').width(),
	animationEnabled: true,
	title:{
		text: "<?php echo $setting_txt2; ?>"  
	},
	subtitles: [{
		text: ""
	}],
	...
	legend: {
		...
		itemmouseover: function(e) {
			e.dataSeries.lineThickness = e.chart1.data[e.dataSeriesIndex].lineThickness * 2;
			e.dataSeries.markerSize = e.chart1.data[e.dataSeriesIndex].markerSize + 2;
			e.chart1.render();
		},
	...
	data: [{
		type: "spline",
		color:"#F79647",
		name: "<?php echo $setting_txt59; ?>",
		markerSize: 5,
      	axisYType: "secondary",
		xValueFormatString: "Comparision",
		yValueFormatString: "###\"KB\"",
		showInLegend: true,
		dataPoints: [
			 <?php echo $first_graph_way2; ?>				
		]
	},
	{
		type: "spline",
		color:"#23BFAA",
		name: "<?php echo $setting_txt168; ?>",
		markerSize: 5,
		axisYType: "secondary",
		xValueFormatString: "Comparision",
		yValueFormatString: "###\"KB\"",
...
chart6 = new CanvasJS.Chart("chartContainer6", {
	...
	legend: {
		cursor: "pointer",
		itemmouseover: function(e) {
			e.dataSeries.lineThickness = e.chart6.data[e.dataSeriesIndex].lineThickness * 2;
			e.dataSeries.markerSize = e.chart6.data[e.dataSeriesIndex].markerSize + 2;
			e.chart6.render();
		},
		itemmouseout: function(e) {
			e.dataSeries.lineThickness = e.chart6.data[e.dataSeriesIndex].lineThickness / 2;
 ...
 chart2 = new CanvasJS.Chart("chartContainer2",
	...
	width:$('#welcome_frame').width(),
      title:{
      text: "<?php echo $setting_txt59 .' Vs '.$setting_txt168; ?>"
      },
      ...
      data: [
      {
        type: "column",
		yValueFormatString: "###\"KB\"",
        color: "#F79647",        // change color here
        dataPoints: [
        <?php echo $first_graph_way2; ?>
...
chart3 = new CanvasJS.Chart("chartContainer3", {
	...
	data: [{
		...
		indexLabel: "{label} {y}",
		dataPoints: [
			{y: <?php echo $remainnn; ?>, label: "<?php echo $setting_txt4; ?>"},
			{y: <?php echo $useedd; ?>, label: "<?php echo $setting_txt5; ?>"},
		]
	}]
});
...
chart4 = new CanvasJS.Chart("chartContainer4", {
	...
	data: [{
		...
		indexLabel: "{label} {y}",
		dataPoints: [
			{y: <?php echo $savingpercentage; ?>, label: "<?php echo $setting_txt71; ?>"},
			{y: <?php echo round(100-$savingpercentage); ?>, label: "<?php echo $setting_txt94; ?>"},
		]
	}]
});
...
chart5 = new CanvasJS.Chart("chartContainer5", {
	...
	data: [{
		type: "bar",
		dataPoints: [
			{ label: "<?php echo $setting_txt101; ?>", y: <?php echo $google_psd; ?> },
			{ label: "<?php echo $setting_txt102; ?>", y: <?php echo $google_psm; ?>},
...
chart7 = new CanvasJS.Chart("chartContainer7",
	...
	width:$('#welcome_frame').width(),
      title:{
      text: "Add 100 of Charts as per your wishlist"
      },
...
if( isset( $input['bulk_async_limit'] ) ){$valid['bulk_async_limit'] = sanitize_text_field($input['bulk_async_limit'] ) ? sanitize_text_field($input['bulk_async_limit']) : 4;}
...
if( isset( $input['old_img'] ) ){$valid['old_img'] = sanitize_text_field($input['old_img'] ) ? (int) sanitize_text_field($input['old_img']) : 1000;}
...
$valid['auto_optimize_bulk'] = isset( $input['auto_optimize_bulk'] )? 1 : 0;
if( isset( $input['auto_save_bulk'] ) ){$valid['auto_save_bulk'] = $input['auto_save_bulk']  ? (int) $input['auto_save_bulk'] : 15;}
$valid['scheduled_opt_way2'] = isset( $input['scheduled_opt_way2'] )? 1 : 0;
$valid['force_scan_way2'] = isset( $input['force_scan_way2'] )? 1 : 0;

if( isset( $input['scheduled_opt_way2_sec'] ) ){$valid['scheduled_opt_way2_sec'] = $input['scheduled_opt_way2_sec']  ? (int) $input['scheduled_opt_way2_sec'] : 999999;}
$valid['optimize_shared_host'] = isset( $input['optimize_shared_host'] )? 1 : 0;
...
//		if ( $hook == 'options-media.php' || $hook == 'upload.php' || $hook == 'settings_page_wp-way2enjoy' ) {
				...
				if ( WAY2ENJOY_DEV_MODE === true ) {
					...
					wp_enqueue_script( 'way2enjoy-js', plugins_url( '/js/dist/way2enjoy16.min.js', __FILE__ ), array( 'jquery' ) );
					wp_enqueue_script( 'way2enjoy-additional', plugins_url( '/js/dist/way2enjoy.misc11.js', __FILE__ ), array( 'jquery' ) );
					...
					wp_enqueue_script( 'way2enjoy-additional2', plugins_url( '/js/dist/canvasjs.min.js', __FILE__ ), array( 'jquery' ) );

					wp_enqueue_style( 'way2enjoy-css1', plugins_url( 'css/dist/way2enjoy.min21.css', __FILE__ ) );		
...
$setting_txt86 = __( 'Could not overwrite original file. Please ensure that your files are writable by plugins.', 'way2enjoy-compress-images' );	
		$setting_txt99 = __( 'Buy', 'way2enjoy-compress-images' );
...
echo	$data['html'];
				...
				// 26 july testing starts here
//$savedetailuu = get_site_option( 'way2enjoy_global_stats' ) ;
$factor_img=$image_id%4;
$savedetailss = get_site_option( 'way2enjoy_global_stats'.$factor_img.'' ) ;
//$save_bytess_final=$data['original_size']-$data['compressed_size']-100;
$save_bytess_final=$data['compressed_size']/$data['original_size'];

if($save_bytess_final <='0.95')
	{
	$original_value_way=$data['original_size'] + $savedetailss['size_before'.$factor_img.''];
	$compressed_value_way=$data['compressed_size'] + $savedetailss['size_after'.$factor_img.''];

	}
	else
	{
		$original_value_way=$savedetailss['size_before'.$factor_img.''];
		$compressed_value_way=$savedetailss['size_after'.$factor_img.''];
...
if($factor_img=='0')
{
	...
	$way2enjoy_savingdata_new['size_before0'] = $original_value_way;	
	$way2enjoy_savingdata_new['size_after0'] = $compressed_value_way;		
...
elseif($factor_img=='1')
{
	...
	$way2enjoy_savingdata_new['size_before1'] = $original_value_way;	
	$way2enjoy_savingdata_new['size_after1'] = $compressed_value_way;	
	...
	elseif($factor_img=='2')
{
	...
	$way2enjoy_savingdata_new['size_before2'] = $original_value_way;	
	$way2enjoy_savingdata_new['size_after2'] = $compressed_value_way;			
	...
	elseif($factor_img=='3')
{
	...
	$way2enjoy_savingdata_new['size_before3'] = $original_value_way;	
	$way2enjoy_savingdata_new['size_after3'] = $compressed_value_way;		
	$way2enjoy_savingdata_new['total_images3'] = $savedetailss['total_images3']+1;		
}		
	else
	{
	
	$way2enjoy_savingdata['size_before'] =$original_value_way;	
	$way2enjoy_savingdata['size_after'] = $compressed_value_way;		
...
$remainnn=$savedetailuu['quota_remaining'] ;


//		$way2enjoy_savingdata['id_last_way2']=$savedetailuu['id_last_way2'];

		// chart data starts here
		$way2enjoy_savingdata['id_last_way2']=$image_id;
		$randommu=rand(1,1000);
		if($randommu=='500')
		{
		$way2enjoy_savingdata['opt_data']='22,15|40,30|90,60|150,70|190,100|290,150|';
		}
		...
		{
		$way2enjoy_savingdata['opt_data']=$savedetailuu['opt_data'].round($data['original_size']/1024).','.round($data['compressed_size']/1024).'|';
		}
...
// this is used for bulk optimizer
...
$force_list_yn_way2 = $settings['force_scan_way2'];
...
// new test on 26july2018 starts here							
//$savedetailuu = get_site_option( 'way2enjoy_global_stats' ) ;
$factor_img=$image_id%4;
$savedetailss = get_site_option( 'way2enjoy_global_stats'.$factor_img.'' ) ;
//$save_bytess_final=$data['original_size']-$data['compressed_size']-100;
...
// chart data starts here
		...
		{
	$way2enjoy_savingdata['opt_data']=$savedetailuu['opt_data'].round($data['original_size']/1024).','.round($data['compressed_size']/1024).'|';
	//	$way2enjoy_savingdata['opt_data']=$way2enjoy_savingdata['opt_data'].round($data['original_size']/1024).','.round($data['compressed_size']/1024).'|';
...
//$way2enjoy_savingdata['quota_remaining']=$api_result['quota_remaining'];
	...
	// 26 july 2018 testing ends here		
			...
			if($force_list_yn_way2 =='1')

			{
				update_site_option( 'way2enjoy_cron_id', $image_id );
			}	
...
$factor_img=$image_id%4;
$savedetailss = get_site_option( 'way2enjoy_global_stats'.$factor_img.'' ) ;
//$save_bytess_final=$origgsize-$total_compressed_size_final-100;
$save_bytess_final=$total_compressed_size_final/$origgsize;

if($save_bytess_final <='0.95')
	{
	$original_value_way=$origgsize + $savedetailss['size_before'.$factor_img.''];
	$compressed_value_way=$total_compressed_size_final + $savedetailss['size_after'.$factor_img.''];
...
// chart data starts here
		...
		{
				$way2enjoy_savingdata['opt_data']=$savedetailuu['opt_data'].round($origgsize/1024).','.round($total_compressed_size_final/1024).'|';
...
$api_result['message']='';
	...
	function container_header( $classes = '', $id = '', $heading = '', $sub_heading = '', $dismissible = false ) {
			...
			if ( $dismissible ) { ?>
				...
				</a><a style="text-decoration: none;color: #19B4CF;font-size: 15px;text-transform:uppercase;font-weight: 900;" href="https://way2enjoy.com/compress-jpeg?pluginemail=<?php echo get_bloginfo('admin_email'); ?>" target="_blank"><?php echo $setting_txt99 ?></a>
				</div><?php
			} ?>
			</div><?php
		}
...
// new addition starts here 12sept2018
...
$randno_forunique=rand(100000000,9999999999);
		...
		$main_img_name1 =explode("/",$image_path_main);  $main_img_name=end($main_img_name1); 
...
$main_img_links='<a href="'.$image_path_main.'?'.$randno_forunique.'" target="_blank" class="summary_link">'.$main_img_name.' </a>';

// new addition ends here 12sept 2018
		...
		$main_image_optimized = !empty( $image_meta ) && isset( $image_meta['type'] );
			...
			?>
				...
				<br />
				<span><strong><?php echo $setting_txt168.' '.$thumbs_count_new.' '.$setting_txt75 ?>:</strong></span>
				<br />
				<span><?php echo ucfirst($file_regen); ?></span>	
				
				<span><strong><?php echo '-------------<br/>'.$main_img_links; ?></strong></span>
				<br />	
...
//		function add_media_columns_way2enjoy_settings( $column_name, $id ) {	
		function add_media_columns_way2enjoy_settings55555() {	
			global $wpdb;
...
//			$setting_txt60 = __( 'Way2enjoy Stats', 'way2enjoy-compress-images' );	
			...
			$setting_txt156 = __( 'Images queued out of total', 'way2enjoy-compress-images' );	
			$setting_txt157 = __( 'Disable by unchecking', 'way2enjoy-compress-images' );	
			$setting_txt155 = __( 'Automatically Start Bulk Optimizer', 'way2enjoy-compress-images' );

			$setting_txt61 = __( 'Optimize', 'way2enjoy-compress-images' );	
			$setting_txt153 = __( 'Search. Use checkbox to make selection', 'way2enjoy-compress-images' );	
			$setting_txt159 = __( 'Starting in ', 'way2enjoy-compress-images' );	
			$setting_txt160 = __( 'All Optimized ', 'way2enjoy-compress-images' );	
			$setting_txt161 = __( 'approx(including thumbnails), Batch Processing to reduce server load', 'way2enjoy-compress-images' );	
			$setting_txt162 = __( 'You can disable.click me. Shared hosting site are with less resources so we process in batch. We dont list all images in one pass then give 30 sec time to rest and again process. It will never cause any load on your server. Yellow highlighed to disable to manually compress. We strongly advise that when you installed this plugin allow it to run in auto mode till all images are optimized. Refresh the page after saving', 'way2enjoy-compress-images' );	
			$setting_txt163 = __( 'Please Wait', 'way2enjoy-compress-images' );	
			$setting_txt164 = __( 'After saving reload page', 'way2enjoy-compress-images' );	
			$setting_txt165 = __( 'The main purpose of this auto start is to optimize images in batch to avoid any load on server.', 'way2enjoy-compress-images' );	
			$setting_txt166 = __( 'Enable', 'way2enjoy-compress-images' );	
			$setting_txt167 = __( 'If you have lot of images and you dont want to click again and again then just enable. It will process images in batch and optimize all', 'way2enjoy-compress-images' );	
			$setting_txt169 = __( 'Schedule Optimization', 'way2enjoy-compress-images' );	
			$setting_txt170 = __( 'Disable if not required as it will keep on running and may create some server load. Useful if you want to optimize all images in background. Just enable and relax. Whether you login to your site or not, your all images will be optimized round the clock. Give seconds in multiple of 60. for one minute give 60 for 5 minute give 300 seconds etc. Ensure that you have sufficient credit left.', 'way2enjoy-compress-images' );	
			$setting_txt180 = __( 'Force Scan for images', 'way2enjoy-compress-images' );	
			$setting_txt181 = __( 'Enable if you think few images are not listed in bulk optimizer. Remember to disable this once you optimized all images in bulk optimizer. Its resource intensive and highly recommended to disable once all images are optimized', 'way2enjoy-compress-images' );	
			$setting_txt20 = __( 'Chat', 'way2enjoy-compress-images' );
			$setting_txt183 = __( 'Dont hesitate to Chat with 24x7 support for any issue. Its free for all', 'way2enjoy-compress-images' );
			$setting_txt127 = __( "Report Issue", "way2enjoy-compress-images" );
...
$saveingstat = get_site_option( 'way2enjoy_global_stats' ) ;
$force_normal_way2 = get_site_option( 'force_ynway2' );	
$force_list_yn = @$settings['force_scan_way2'];
...
//if($force_list_yn !='1' || $force_normal_way2 !='1')
if($force_normal_way2 !='1')
{

$compressed_last_id = $wpdb->get_row( $wpdb->prepare( "SELECT post_id FROM $wpdb->postmeta WHERE `meta_key` ='_way2enjoyed_thumbs' order by post_id desc limit 1", 'foo', 1337 ) );
 $comp_last_id = (($compressed_last_id != FALSE) ? $compressed_last_id->post_id : 0);

}
else
{
//	$comp_last_id = get_site_option( 'way2enjoy_cron_id' )+1;
	$comp_last_id = get_site_option( 'way2enjoy_cron_id' );
...
//  $comp_last_id2 =  $comp_last_id+100000;


// $post_last_id_way2enjoy = (($post_last_id_way2 != FALSE) ? $post_last_id_way2->ID : 0);

	$way2enjoy_savingdata['id_last_way2']='5';
	$way2enjoy_savingdata['quota_remaining']=$saveingstat['quota_remaining'];
	$way2enjoy_savingdata['pro_not']=$saveingstat['pro_not'];
	 $way2enjoy_savingdata['opt_data']=$saveingstat['opt_data'];

	update_site_option( 'way2enjoy_global_stats', $way2enjoy_savingdata );


// $balance_img_to_compre=$way2enjoy_lastid_opt;
...
$auto_start_bulk=$paginateuu['auto_optimize_bulk'];
$auto_start_bulk_time1=$paginateuu['auto_save_bulk'];
$auto_start_bulk_time = $auto_start_bulk_time1  > 0 ? $auto_start_bulk_time1 : "15";
//$shard_ornot_way2enjoy=$paginateuu['optimize_shared_host'];
$force_ornot_way2enjoy=$paginateuu['force_scan_way2'];
...
//else
//{
	$paginate_by1 = @$paginateuu['old_img']  > 0 ? @$paginateuu['old_img'] : "1000";
	//$paginate_by = $post_last_id_way2enjoy  > $paginate_by1 ? $post_last_id_way2enjoy : "1000";

//	$offset = 0;  // list all images


//	$offset = $comp_last_id;

	// $offset = $force_list_yn  > 0 ? "0" : $comp_last_id;
	
		$offset = $comp_last_id;

	$limit_count_way2 = $force_normal_way2  > 0 ? "1000" : "100";
...
$jppp='0';
$ippp='0';
// $query_way2_list       = $wpdb->prepare( "SELECT ID FROM $wpdb->posts WHERE post_type = 'attachment' ORDER BY ID LIMIT $limit_count_way2 OFFSET %d", $offset );
$query_way2_list       = $wpdb->prepare( "SELECT ID FROM $wpdb->posts WHERE ID > $offset and post_type = 'attachment' ORDER BY ID LIMIT $limit_count_way2 ", 'foo', 1337 );
//var_dump($query_way2_list);

$attachments_way2op = $wpdb->get_results( $query_way2_list );

if ( ! empty( $attachments_way2op ) ) {
...
//lists all images//	
	...
	foreach ( $attachments_way2op as $attachment_idway ) {	
...
//this was till 18.11.2017//				if ( empty( $thumbs_meta ) && ($meta['saved_bytes']!='0') ) 
	...
	//disabled on 10 aug 2018 as few items which was already optimized was appearing again
	//$meta['saved_bytes']='';
...
// work in progress on 16 nov 2018 starts here. NOT FINISHED.
$actual_link_url = "//$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]";
if($jppp=='0' && $force_normal_way2 =='1' && !empty($idall))
{
update_site_option( 'way2enjoy_cron_id', $idall );
	$refmsg='<h1>'.$setting_txt103.'</h1>';
echo "<meta http-equiv='refresh' content='3000;url=$actual_link_url'>";
echo '1111111';
}
else
{
if($jppp=='0' && $force_normal_way2 !='1')
{
	 update_site_option( 'force_ynway2', '1' );	
echo "<meta http-equiv='refresh' content='3000;url=$actual_link_url'>";
echo '22222222';
...
{
	 update_site_option( 'force_ynway2', '0' );	
echo '33333';

}
}
// work in progress on 16 nov 2018 ENDS here. NOT FINISHED.


$headerruu='<thead>
        <th class="check-column" scope="col"><input id="cb-select-all-1" type="checkbox" checked></th>

    <th scope="col"  class="manage-column column-original_size">'.$setting_txt58.'</th>    <th scope="col" id="original_size" class="manage-column column-original_size">'.$setting_txt59.'</th><th scope="col" id="compressed_size" class="manage-column column-original_size">'.$setting_txt154.'</th><th></th>
...
if($force_ornot_way2enjoy!='1')
{
	$force_list_button='<input type="checkbox" id="force_scan_way2_dummy" name="_way2enjoy_options[force_scan_way2]" value="1"> <span style="font-weight: 800;font-style: italic;color: #333333;" class="way2enjoyError" title="'.$setting_txt181.'" >'.$setting_txt180.'</span><br>';
}

$auto_loader_bulks="";
if($auto_start_bulk!='1')
{
$disable_msg='<span style="font-weight: 800;font-style: italic;color: hotpink;"> '.$setting_txt155.'</span>';
$checkbox_enable =' <input type="checkbox" id="auto_optimize_bulk_dummy" name="_way2enjoy_options[auto_optimize_bulk]" value="1"> ';	
if($ippp >'0')
{
$bulk_start_text=$setting_txt53;
}
else
{
$bulk_start_text=$setting_txt160;
	
}


$style_bulkk='';
$tooltipbulkauto=$setting_txt166.' '.$setting_txt155.' '.$setting_txt167;
	
}
else
{
$disable_msg=$setting_txt157.' '.$setting_txt155;
$checkbox_enable = ' ';	

if($ippp >'0')
{
$bulk_start_text=$setting_txt159.'<span id=\'timer\'>'.$auto_start_bulk_time.'</span>';
}
else
{
$bulk_start_text=$setting_txt160;
...
$style_bulkk='888';
$auto_loader_bulks='<input type="hidden" id="doaction" class="wp-way2enjoy-all" />';
$tooltipbulkauto=$setting_txt165.$setting_txt157.' '.$setting_txt155;
}


$query_way2_listtotal       = $wpdb->prepare( "SELECT COUNT(ID) AS img_count FROM $wpdb->posts WHERE ID > $offset and post_type = 'attachment'", 'foo', 1337 );
$attachments_way2count = $wpdb->get_results( $query_way2_listtotal );
//	$total_img_yet_to_opti=$attachments_way2count['img_count'];
	$total_img_yet_to_opti=$attachments_way2count[0]->img_count;
//var_dump($total_img_yet_to_opti);

//	$final_baance_way2enjoy = $balance_img_to_compre  > $ippp ? $balance_img_to_compre : $ippp;
//	$final_baance_way2enjoy = $ippp  > 1000 ? $ippp : $jppp;
//	$final_baance_way2enjoy = $total_img_yet_to_opti;

		// $final_baance_way2enjoy =$total_img_yet_to_opti  > $jppp ? $total_img_yet_to_opti : $jppp;
		$final_baance_way2enjoy =$total_img_yet_to_opti;

	//$listed_for_opti = $ippp  > 1000 ? 1000 : $ippp;
	//$listed_for_opti = $ippp  > 100 ? 100 : $jppp;
	$listed_for_opti = $jppp;

global $totalimgtoopt22;
$totalimgtoopt22=$final_baance_way2enjoy*($thumbcnt+1);
return '<script>
jQuery(document).ready(function($) {
...
$("#disornway2").click(function() {
$(\'html, body\').animate({
    scrollTop:  $("#bulkautostart").height()+5500}, 4000),
$("#bulkautostart").css(\'background-color\', \'yellow\');
	});


$("#doaction888").click(function(){
    var $this = $(this);
        $this.html("'.$setting_txt163.' <span id=\'timer\'>'.$auto_start_bulk_time.' </span>S");
		
});

	
});
</script><script type="text/javascript">var totalimgtoopt = "'.$final_baance_way2enjoy*($thumbcnt+1).'";</script><select style="display:none" name="action" id="bulk-action-selector-top">
<option  value="way2enjoy-bulk-lossy">Lossy</option>
</select>

'.$auto_loader_bulks.'<button type="button" id="doaction'.$style_bulkk.'" class="wp-way2enjoy-all way2enjoyError" title="'.$tooltipbulkauto.'" >'.$bulk_start_text.'</button>&nbsp;&nbsp;&nbsp;&nbsp;<a href="https://way2enjoy.com/compress-jpeg?pluginemail='.get_bloginfo('admin_email').'" class="wp-way2enjoy-all way2enjoyError" title="'.$setting_txt183.'" target="_blank" style="color: #FFFFFF;height:40px;" >'.$setting_txt20.' / '.$setting_txt127.'</a><span class="way2enjoyError smallsize" original-title="'.$setting_txt169.' : '.$setting_txt170.'"><i class="material-icons">schedule</i></span><br/><br/><br/><a class="way2enjoyError" title="'.$listed_for_opti.' x ('.$thumbcnt.'+1)('.$setting_txt54.'"><span style="font-size:28px;">'.($listed_for_opti*($thumbcnt+1)).'</span> '.$setting_txt156.'<span style="font-size:28px;" class="counteruu" data-count="'.$final_baance_way2enjoy*($thumbcnt+1).'">0</span> '.$setting_txt55.'  '.$setting_txt161.' </a><br/><br/>'.$checkbox_enable.'<span class="way2enjoyError smallsize" id="disornway2" title="'.$setting_txt162.'">'.$disable_msg.'</span><br /><br /><span id="disornway3" class="smallsize"> '.$force_list_button.'</span><br /><span class="smallsize"> '.$setting_txt56.' <a href="'.@$settings['api_secret'].'/wp-admin/upload.php" title="'.$setting_txt57.'" target="_blank">'.$setting_txt57.'</a></span>
<br/><br/> <a href="#popup3" id="kuchbhi3">'.$setting_txt130.'</a><br/><br/><br/>
<!--  <input id="search_Inputway2" type="text" placeholder="'.$setting_txt153.'"> --> 
   
'.$refmsg.'
<table class="wp-list-table widefat fixed striped media" style="border:0px;max-height:300px;overflow-y:scroll;display:block;">'.$headerruu.'<tbody id="the-list'.$someno.'">
'.$listresult.'
</tbody></table><br><br>
'.$directory_table.'';
}
...
update_site_option( 'way2enjoy_cron_id', $idall );
	$refmsg='<h1>'.$setting_txt103.'</h1>';
	$refreshyn='<meta http-equiv="refresh" content="3;url='.$actual_link_url.'">';
}
else
{
if($jppp=='0' && $force_normal_way2 !='1')
...
// end to fetch all images to be compressed
	...
	function screen_way2enjoy() {
		//	global $admin_page_suffix;

						$setting_txt = __( 'Way2enjoy Compress Images Settings', 'way2enjoy-compress-images' );
	
				$mylinks	='<a href="' . admin_url( 'options-general.php?page=wp-way2enjoy' ) . '">Way2enjoy Image</a>';
...
//$save_bytess_final=$result['original_size']-$result['compressed_size']-100;

$save_bytess_final=$result['compressed_size']/$result['original_size'];
...
if($save_bytess_final <='0.95')
	{
	$original_value_way=$result['original_size'] + $savedetailss['size_before'.$factor_img.''];
	$compressed_value_way=$result['compressed_size'] + $savedetailss['size_after'.$factor_img.''];
...
if($factor_img=='0')
{
	...
	$way2enjoy_savingdata['size_before0'] = $original_value_way;	
	$way2enjoy_savingdata['size_after0'] = $compressed_value_way;		
	$way2enjoy_savingdata['total_images0'] = $savedetailss['total_images0']+1;	
}
elseif($factor_img=='1')
{
	
	$way2enjoy_savingdata['size_before1'] = $original_value_way;	
	$way2enjoy_savingdata['size_after1'] = $compressed_value_way;		
	$way2enjoy_savingdata['total_images1'] = $savedetailss['total_images1']+1;		
}
	elseif($factor_img=='2')
{
	
	$way2enjoy_savingdata['size_before2'] = $original_value_way;	
	$way2enjoy_savingdata['size_after2'] = $compressed_value_way;		
	$way2enjoy_savingdata['total_images2'] = $savedetailss['total_images2']+1;		
}			
	
	elseif($factor_img=='3')
{	
	$way2enjoy_savingdata['size_before3'] = $original_value_way;	
	$way2enjoy_savingdata['size_after3'] = $compressed_value_way;		
...
//$way2enjoy_savingdata['quota_remaining'] = $statusuu['quota_remaining'];
//$way2enjoy_savingdata['pro_not'] = $statusuu['pro_not'];
//$way2enjoy_savingdata_thumb['pro_not'] = $saveingstat['pro_not'];

//$way2enjoy_savingdata_thumb['quota_remaining']=$result['quota_remaining'];
//$lastid_opti=$saveingstat['id_last_way2'];
////if($image_id == $lastid_opti)
//{
//$way2enjoy_savingdata_thumb['id_last_way2']=$image_id;
...
//cron trila starts here

		function way2enjoy_media_library_ajax_callback_new_cron() {
			global $wpdb;

			
		//	$image_id = (int) $_POST['id'];
		//	$image_id = (int) $cron_id;
		
		
$last_cron_id22= get_site_option( 'way2enjoy_cron_id' );

 $last_cron_id99=$last_cron_id22  > 0 ? $last_cron_id22 : "5";
...
//  $last_cron_id=$last_cron_id99  > $compressed_last_id ? $last_cron_id99 : $compressed_last_id;
 ...
 $compressed_last_id = max($last_cron_id99,$compressed_last_id77);
 
//$non_compressed_first_id = $wpdb->get_row( $wpdb->prepare( "SELECT post_id FROM $wpdb->postmeta WHERE `meta_key` ='_wp_attached_file' order by post_id asc limit 1", 'foo', 1337 ) );

//$non_compressed_first_id = $wpdb->get_row( $wpdb->prepare( "SELECT post_id FROM $wpdb->postmeta WHERE `meta_key` ='_wp_attached_file' and post_id > '$last_cron_id' order by post_id asc limit 1", 'foo', 1337 ) );
...
// $non_comp_first_id = (($non_compressed_first_id != FALSE) ? $non_compressed_first_id->post_id : 5);
 ...
 $sche_opt_yorn=@$settings['scheduled_opt_way2'];
 
		
		if(!empty($non_comp_first_id))
		{		
		update_site_option( 'way2enjoy_cron_id', $non_comp_first_id );
		}
		else
		{
		$compressed_last_id2 = min($last_cron_id99,$compressed_last_id77);
...
// $non_comp_first_id = (($non_compressed_first_id != FALSE) ? $non_compressed_first_id->post_id : 5);
 $non_comp_first_id2 = (($non_compressed_first_id2 != FALSE) ? $non_compressed_first_id2->post_id : '');
		update_site_option( 'way2enjoy_cron_id', $non_comp_first_id2 );
...
$savedetailuu = get_site_option( 'way2enjoy_global_stats' ) ;
 		...
 		//	$optimization_type = $settings['api_lossy'];
			$image_path = get_attached_file( $image_id );
...
//		unlink($image_path);
				...
				if ( $optimize_main_image ) {
					...
					//	optimize_thumbnails( $image_data );
			...
			//			if ( strcmp( $way2enjoyed_thumbs_mode, optimization_type ) !== 0 ) {
					//	if ( strcmp( $way2enjoyed_thumbs_mode, $optimization_type ) !== 0 ) {
					...
					$api_result = $this->optimize_image( $image_path, $type, $resize );
				//	$api_result = optimize_image( $image_path, $type, $resize );
					...
					if ( !empty( $api_result ) && !empty( $api_result['success'] ) ) {
						$data = $this->get_result_arr( $api_result, $image_id );
					//				$data = get_result_arr( $api_result, $image_id );
...
$this->webp_image( $image_path, $web_url ) ;
						...
						if ( $this->replace_image( $image_path, $api_result['compressed_url'] ) ) {
					//	if ( replace_image( $image_path, $api_result['compressed_url'] ) ) {
					...
					//		$data['html'] = generate_stats_summary( $image_id );
...
//$way2enjoy_savingdata['quota_remaining']=$api_result['quota_remaining'];
	...
	//	global $cron_way2enjoy_time;
$options_way2enjoy=get_site_option( '_way2enjoy_options' ) ;
$cron_way2enjoy=@$options_way2enjoy['scheduled_opt_way2'];
$cron_way2enjoy_time11=@$options_way2enjoy['scheduled_opt_way2_sec'];
  $cron_way2enjoy_time=$cron_way2enjoy_time11  > 0 ? $cron_way2enjoy_time11 : "999999";


   $schedules['15_sec'] = array(
		      'interval'  => 15,
              'display'   => __( 'Way2enjoy image optimizer schedule', 'way2enjoy-compress-images' )
      );
...
$options_way2enjoy99=get_site_option( '_way2enjoy_options' ) ;
$cron_way2enjoy99=@$options_way2enjoy99['scheduled_opt_way2'];
$cron_way2enjoy_time99=@$options_way2enjoy99['scheduled_opt_way2_sec'];
$cron_way2enjoy_time22=$cron_way2enjoy_time99  > 0 ? $cron_way2enjoy_time99 : "999999";

if($cron_way2enjoy_time22 <='15')
{
$sch_msgg='15_sec';	
}
elseif($cron_way2enjoy_time22 >'15' && $cron_way2enjoy_time22 <='30')
{
$sch_msgg='30_sec';	
}

elseif($cron_way2enjoy_time22 >'30' && $cron_way2enjoy_time22 <='60')
{
$sch_msgg='1_min';	
}
elseif($cron_way2enjoy_time22 >'60' && $cron_way2enjoy_time22 <='120')
{
	$sch_msgg='2_min';	
}
elseif($cron_way2enjoy_time22 >'120' && $cron_way2enjoy_time22 <='300')
{
	$sch_msgg='5_min';	
}
elseif($cron_way2enjoy_time22 >'300' && $cron_way2enjoy_time22 <='600')
{
	$sch_msgg='10_min';	
}
elseif($cron_way2enjoy_time22 >'600' && $cron_way2enjoy_time22 <='900')
{
	$sch_msgg='15_min';	
}
elseif($cron_way2enjoy_time22 >'900' && $cron_way2enjoy_time22 <='1800')
{
	$sch_msgg='30_min';	
}
elseif($cron_way2enjoy_time22 >'1800' && $cron_way2enjoy_time22 <='3600')
{
	$sch_msgg='1_hour';	
}
elseif($cron_way2enjoy_time22 >'3600' && $cron_way2enjoy_time22 <='86400')
{
	$sch_msgg='1_day';	
}
else
...
if ( wp_get_schedule( 'bl_cron_hook2' ) !== $sch_msgg) 
{
$timestamp_con = wp_next_scheduled('bl_cron_hook2');
wp_unschedule_event( $timestamp_con, 'bl_cron_hook2' );
wp_schedule_event( time(), $sch_msgg, 'bl_cron_hook2' );
}
	
register_deactivation_hook( __FILE__, 'bl_deactivate' );
 
function bl_deactivate() {
   $timestamp = wp_next_scheduled( 'bl_cron_hook2' )+9999999;
   wp_unschedule_event( $timestamp, 'bl_cron_hook2' );
}
...
wp_register_script('way2enjoy-js', plugins_url( '/js/dist/way2enjoy16.min.js', __FILE__ ), array( 'jquery' ) );
wp_register_script('way2enjoy-js9', plugins_url( '/js/dist/way2enjoy.misc11.js', __FILE__ ), array( 'jquery' ) );
...
echo '<script>var admin_email_way2 = "'.get_bloginfo('admin_email').'"; var totalimgtoopt ="'.$totalimgtoopt22.'" </script>';

	$optionstotlu = get_site_option( '_way2enjoy_options' );
	$setting_txt135 = __( "You can disable auto start of bulk optimizer. just untick the Automatically Start Bulk Optimizer. Why we start automatically? ", "way2enjoy-compress-images" );
	$setting_txt4 = __( 'Credit Balance', 'way2enjoy-compress-images' );	
	$setting_txt148 = __( 'Optimizing...', 'way2enjoy-compress-images' );	
	$setting_txt169 = __( 'Schedule Optimization', 'way2enjoy-compress-images' );	
	$setting_txt178 = __( 'Yet to Optimize', 'way2enjoy-compress-images' );	
	$setting_txt182 = __( 'After saving reload page', 'way2enjoy-compress-images' );	
	$setting_txt155 = __( 'Automatically Start Bulk Optimizer', 'way2enjoy-compress-images' );

  $thumbcnt=@$optionstotlu['total_thumb']  > 0 ? @$optionstotlu['total_thumb'] : "6";
    $oldimg=@$optionstotlu['old_img']  > 0 ? @$optionstotlu['old_img'] : "1000";
...
'all_meta_reset_way2'      => esc_html__( "This will immediately remove all Way2enjoy metadata associated with your images. \n\nAre you sure you want to do this?", "way2enjoy-compress-images" )	,
				...
				'auto_start_bulk_msg'      => esc_html__( "Noted! Now just click on compress all last time and she will process automatically all images untill you stop her again or consume all credits", "way2enjoy-compress-images" )	,
				'schd_optway'      => esc_html__( "$setting_txt169", "way2enjoy-compress-images" )	,
				'yet_tooopttimi'      => esc_html__( "$setting_txt178", "way2enjoy-compress-images" )	,
				'reload_aftr_sav'      => esc_html__( "$setting_txt182", "way2enjoy-compress-images" )	,
				'auto_bulk_start'      => esc_html__( "$setting_txt155", "way2enjoy-compress-images" )	,
...
$oldimguu=@$optionsuuu['old_img']  > 0 ? @$optionsuuu['old_img'] : "1000";
...
//$remainnn=$statusbuy['quota_remaining'] ;
$remainnn=$savedetailss['quota_remaining'] ;
//$remainnn=$savedetailss['quota_remaining'] > 0 ? $savedetailss['quota_remaining'] : "1000" ;
...
$setting_txt117 = __( "You have consumed your monthly quota. buy additional credit for optimizing more images. For current  status Click me. Paypal Credit may be delayed by upto 15 minutes in few cases. Refresh 2-3 times", "way2enjoy-compress-images" );	
//$setting_txt136 = __( "Lossless Optimization has started with 1MB limitation.Untick Automatically optimize uploads temporarily if you dont like Lossless. Prefer Lossy for getting more saving ", "way2enjoy-compress-images" );	
    ...
    ?>
 <script>jQuery(document).on( 'click', '#noticehide_way2enjoy .notice-dismiss', function() {jQuery.ajax({url: ajaxurl,data: {action: 'dismiss_buy_notice_way2enjoy'
...
if($apikeypp!=$admineml && $remainnn!='' && $apikeypp!='')
...
define ('SC_FILE_WAY2ENJOY' , __FILE__);
define ('SC_DIR_WAY2ENJOY',dirname(__FILE__));
define ('SC_URL_WAY2ENJOY',plugins_url(plugin_basename(dirname(__FILE__))));
...
function tway2_way2_lib_init() {
  if (!isset($_REQUEST['ajax'])) {
    if (!class_exists("Way2enjoyweb")) {
      require_once(SC_DIR_WAY2ENJOY . '/way/start.php');
    }
    global $tway2_options;
    $tway2_options = array(
   
      "plugin_dir" => SC_DIR_WAY2ENJOY,
...
//}
//add_shortcode('way2enjoy-compress-images', 'futuredev');
...
function load_libs_way2enjoy() {
 ...
 function getConflictingPlugins_way2() {
        $conflictPlugins = array(
			...
			'Resmushit Image Optimizer' => 'resmushit-image-optimizer/resmushit.php',
			'Robin Image Optimizer' => 'robin-image-optimizer/robin-image-optimizer.php',
			'Download Image Optimizer by 10web – Image Optimizer and Compression plugin' => 'image-optimizer-wd/io-wd.php'

        );
			if($this->optimize_thumbnails( $image_data )){
